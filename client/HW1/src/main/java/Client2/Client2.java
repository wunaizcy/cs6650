...
public class Client2 {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    CountDownLatch countDownLatchTotal = new CountDownLatch(
        (int) (numberThreads / 4 + numberThreads + numberThreads / 10));
    ...
    for (int i = 0; i < numberThreads / 4; i++) {
      ...
      Runnable thread = () -> {
        Client2Phase phase1 = new Client2Phase();
        try {
    ...
    for (int j = 0; j < numberThreads; j++) {
      ...
      Runnable thread = () -> {
        Client2Phase phase2 = new Client2Phase();
        try {
    ...
    for (int m = 0; m < numberThreads / 10; m++) {
      ...
      Runnable thread = () -> {
        Client2Phase phase3 = new Client2Phase();
        try {
    ...
    System.out.println("Wall Time: " + (Client2Phase.totalRequest/totalTime));
    System.out.println("Mean response time: " + Client2Phase.findAverage());
    System.out.println("Median response time: " + Client2Phase.response
        .get(Client2Phase.response.size() / 2));
    System.out.println("Min response time: " + Client2Phase.response.get(0));
    System.out.println("Max response time: " + Client2Phase.response.get(
        Client2Phase.response.size()-1));
    System.out.println("p99 response time: " + Client2Phase.percentile(99));
  }
}
